...
if($_POST['status']=='course_update')
{
		...
		$con->query("UPDATE `courses` SET `course_name`='$course_name',`short_name`='$short_name',`duration`= '".$_POST['duration']."' WHERE id = $id");
		echo '<script>alert("Course Update successfully saved.");location.href="courses.php"</script>';
}
?>
		...
		<form action="" method="post" class="">
		    ...
		    ?>
        				...
        				<select class="form-control" name="categary" required="">
        				    ...
        				    <?
        				        ...
        				        while($row = $get->fetch_assoc()){
        				            $select = $category_id == $row['id'] ? 'selected' : '';
        			...
        			<div class="form-group col-md-4">
        				...
        				<input type="text" name="course_code" class="form-control" placeholder="Enter Course code" required="" value="<?= $courseCode?>">
        			</div>
        			...
        			<!--	<input type="number" name="fee" class="form-control" placeholder="Enter Course Fees" required="" value="<?=$fee?>">-->
        			<!--</div>-->
