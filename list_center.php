...
<div class="container">
                ...
                <form method="get" class="form-inline">
                    ...
                    <div class="form-group col-md-5">
                        ...
                        <select class="form-control" name="city_id" required>
                            <option value="">--Select--</option>
                            <?
                                $state = $con->query("SELECT * FROM city");
                                while($s = $state->fetch_assoc())
                                {
                                    echo '<option value="'.$s['id'].'">'.ucwords($s['city_name']).'</option>';
                                }
                            ?>
                        </select>
                    </div>
    ...
    <?
        if(isset($_GET['state_id']) && isset($_GET['city_id'])){
    ?>
    <div class="container-fluid" style="margin-top:30px">
        <table class="table table-bordered">
            ...
            <tbody>
                <?  
                    $get = $con->query("SELECT * FROM centers where status = 1 AND state_id = '".$_GET['state_id']."' AND city_id = '".$_GET['city_id']."' order by id desc");
