...
<div class="container">
    ...
    <div class="box box-primary">
	   <h1 class="text_heading text-center">Addmission  <span class="highlight_color">Form</span></h1>
	    <div class="box-body">
		<form action="razorpay/student-process.php" method="post" id="student-form" enctype="multipart/form-data">
			...
			<div class="form-group col-xs-6 col-md-12 col-lg-12">
				<label>Name</label>
			...
			<!--	<input type="text" class="form-control" name="enrollment_no" placeholder="Enter Enrollment" required="" value="">-->
			<!--</div>-->
			<div class="form-group col-xs-12 col-md-12 col-lg-12">
			...
			<div class="form-group col-xs-12 col-md-12 col-lg-12">
                ...
                <button class="btn btn-primary" type="button" style="align-item:center;" id="student-add"><i class="fa fa-save"></i> Submit</button>
			</div>
		</form>
	</div>
    </div>
...
<script src="https://checkout.razorpay.com/v1/checkout.js"></script>
  <script>
        $(document).on('click','#student-add',function(rr){
            rr.preventDefault();
            var form = $(this).closest('form');
            var i = false;
            var message = `Fill Required Fields.\n`;
            form.find('input:required,select:required').each(function(){
                i = true;
                var current_msg = $(this).attr('placeholder') || '';
                if(current_msg)
                message = `${message} \t${current_msg}\n`;
                // console.log(this);
            })
            if(i){
                alert(message);
            ...
            var amount = 800;
            var options = {
                "key": "rzp_live_dZX94YrbtrpPKE",
                "amount": amount * 100, // Amount is in currency subunits (e.g., cents)
                "currency": "INR",
                "name": "SHIVAJI MAHARAJ YOUTH COMPUTER ACADEMY", 
                "description": $(form).find('[name="name"]').val() + " Addmission Fee",
                ...
                "prefill": {
                    ...
                    "contact" : $(form).find('[name="mobile"]').val()
